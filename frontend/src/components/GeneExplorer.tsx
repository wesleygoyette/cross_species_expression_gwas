...
export function GeneExplorer() {
  ...
  const handleZoomIn = () => {
    const newZoomLevel = Math.min(zoomLevel * 2, 8);
    // Calculate the gene center position in base coordinates
    ...
    // Center the new view on the gene center
    const newViewStart = geneCenterPosition - (newWindowSize / 2);
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case '=':
          if (e.ctrlKey || e.metaKey) {
            e.preventDefault();
            handleZoomIn(); // This now uses center-based zooming
          }
          break;
        case '-':
          if (e.ctrlKey || e.metaKey) {
            e.preventDefault();
            handleZoomOut(); // This now uses center-based zooming
          }
          break;
        case '0':
          if (e.ctrlKey || e.metaKey) {
            ...
            <Card className="p-6 bg-card border-border">
              <Tabs defaultValue="tracks" className="w-full">
                ...
                <TabsContent value="tracks" className="space-y-4">
                  ...
                  {/* Zoom Controls */}
                  <div className="flex items-center justify-between gap-4 p-3 bg-secondary/30 rounded-lg border border-border">
                    <div className="flex items-center gap-2">
                      <Button
                        size="sm"
                        variant="outline"
                    ...
                    <div className="flex items-center gap-4 text-xs text-muted-foreground">
                      <span>Window: {(currentWindowSize / 1000).toFixed(1)} kb</span>
                      <span>•</span>
                      <span>Resolution: {zoomLevel >= 4 ? 'High' : zoomLevel >= 1 ? 'Medium' : 'Low'}</span>
                      {zoomLevel > 1 && (
                        <>
                          <span>•</span>
                          <span>View: {((clampedViewStart / geneCoords.length) * 100).toFixed(0)}%</span>
                        </>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="space-y-6" ref={trackRef} onWheel={handleWheel}>
                    <div className="text-xs text-muted-foreground mb-2 flex items-center gap-2">
                      ...
                      Hold Ctrl/Cmd + scroll to zoom, arrow keys to pan when zoomed, Ctrl/Cmd + 0 to reset
                    </div>
                                  ...
                                  return enhancerScreenPos >= -5 && enhancerScreenPos <= 105 ? (
                                    <TooltipProvider key={i}>
                                      <Tooltip>
                                        <TooltipTrigger asChild>
                                          <div
                                            className={`absolute top-8 rounded-full cursor-help transition-all duration-200 ${
                                              hasScore ? 'bg-[var(--data-orange)]' : 'bg-[var(--data-orange)]/30'
                                            }`}
                                            style={{ 
